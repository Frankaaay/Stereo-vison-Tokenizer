...
def test_structure_matches_post_fusion_design(production_model):
    ...
    assert not hasattr(production_model.decoder, "dec_temporal_transformer")
    assert not hasattr(production_model.decoder, "temporal_unpatchify")
    assert not hasattr(production_model.decoder, "to_depth")

    decoder = production_model.decoder
    assert decoder.to_pixels[0].out_features == 3 * 4 * 16 * 16
    assert decoder.to_disparity[0].out_features == 4 * 16 * 16
