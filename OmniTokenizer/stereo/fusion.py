...
class StereoFusionOutput:
    """StereoFusion result and the tensors needed to audit its matching path."""
...
class StereoFusion(nn.Module):
    ...
    Inputs use channels-last layout ``[B, V, T, H, W, D]``. One set of
    projection weights is shared by every view. ``search_radii`` only changes
    the candidate mask for each view.
    """
    ...
    def forward(
        ...
        if left.shape != right.shape:
            raise ValueError(
                f"left and right feature shapes must match, got "
