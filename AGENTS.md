...
- 禁止在同一轮回复中使用 `sleep`、`tail -f`、轮询循环或连续工具调用等待训练、评估、预处理、
  checkpoint 验证等长任务完成。除非用户在当前轮明确要求持续监控，否则启动健康检查完成后必须
  报告当前状态和 ETA，并结束该轮，不得为了等待状态变化而保持回复不结束。
- 长任务启动后只做一次启动健康检查：确认目标进程或 tmux 存在、目标 GPU 已按预期占用、日志没有
  立即异常，并读取第一个已经存在的 step 或 heartbeat。尚无足够真实吞吐时使用历史实测区间作为
  初估，不得通过反复查询等待第二个采样点；后续只有用户主动询问状态时才刷新 ETA。
- 用户询问运行状态时，每轮只做一次状态快照，不等待状态变化。快照仅包含回答问题所需的最小事实：
  tmux/进程、GPU 占用、最新 step 和必要 loss、首个根因 traceback、checkpoint/result 等关键产物。
  尽量把检查合并成一个远端命令；单轮原则上不超过 3 次远程查询。
- 禁止在几十秒或数分钟内重复确认同一个未变化事实。SSH 255、连接中断或返回缺失时，按“运行事实与
  远端写操作”规则先做一次只读落地核对；只有结果仍不足以判断时，才允许额外再检查一次。
- 仅在任务预计 2 分钟内完成，或用户明确要求当前轮等待/持续监控时，允许进行有限复查；仍应合并查询、
  及时汇报，不得无限轮询。诊断问题时先回复已经确认的结论，再说明尚需执行的最小检查，不要把所有
  后续取证和修复都捆绑在一次回复中。
