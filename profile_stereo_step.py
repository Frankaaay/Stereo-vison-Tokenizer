...
def _window_stats(
    ...
    values = [
        ...
        and (temporal_mode is None or row["temporal_mode"] == temporal_mode)
    ]
    if not values:
        return {"count": 0}
...
def validate_profile_args(args) -> None:
    ...
    if args.profile_lpips_gt_cache:
        raise ValueError(
            "alternating single/four profiling forbids a shared LPIPS GT cache"
        )
...
def main() -> None:
    ...
    for module in model.modules():
        if isinstance(module, PEG):
            ...
            peg_count += 1
    if peg_count == 0:
        raise RuntimeError("expected at least one spatial PEG module")
    ...
    result = {
        ...
        "wall_windows_by_temporal_mode": {
            ...
            for temporal_mode in ("four_frame", "single_frame")
        },
